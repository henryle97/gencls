...
def accuracy(pred, target):
    """ Calculate accuracy
    Args:
        pred (torch.Tensor | np.array): The model prediction with the shape (BxC)
        target (torch.Tensor | np.array): The target of each prediction with shape (Nx1) or (N,)
    Returns:
        (float | np.array ): accuracy
